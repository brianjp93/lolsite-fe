...
export function rankParticipants(participants: FullParticipantType[]) {
  ...
  if (participants.length === 10) {
    const temp100 = participants.filter((item) => {
      return item.team_id === 100
    })
    const temp200 = participants.filter((item) => {
      return item.team_id === 200
    })
    const team100 = rankTeam(temp100)
    const team200 = rankTeam(temp200)
...
export function rankTeam(team: FullParticipantType[]) {
  ...
  const newteam: I[] = []
  for (const p of team) {
    const f1_impact = (p.stats.total_damage_dealt_to_champions / f1) * f1_weight
    const f2_impact = (p.stats.damage_dealt_to_objectives / f2) * f2_weight
    const f3_impact = (p.stats.damage_dealt_to_turrets / f3) * f3_weight
    const f4_impact = (p.stats.kills / f4) * f4_weight
    const f5_impact = (p.stats.vision_score / f5) * f5_weight
    const f6_impact = (p.stats.total_heal / f6) * f6_weight
    const f7_impact = (p.stats.time_ccing_others / f7) * f7_weight
    const f8_impact = (p.stats.assists / f4) * f8_weight

    const d1_impact = (p.stats.deaths / d1) * d1_weight
    ...
    const detriment = [d1_impact].reduce((a, b) => a + b)
  ...
  const retTeam: AppendParticipant[] = [...team]
