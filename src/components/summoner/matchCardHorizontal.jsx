...
import { ParticipantItems } from "../utils";
...
function MatchCard(props) {
  ...
  const getDamagePercentage = (part, match) => {
    let perc =
      (part.stats.total_damage_dealt_to_champions / getMaxDamage(match)) * 100;
  ...
  const getTeam = (team_id) => {
    let team = participants.filter(
      (participant) => participant.team_id === team_id
    );
    return (
      <div>
        {team.map((part) => {
                  ...
                  <small>
                    {part.puuid !== "0" && (
                      <Link
                        ...
                        title={part.summoner_name}
                        className={`silent`}
                        href={`/${props.region}/${part.summoner_name}/`}
                      >
                        {formatName(part.summoner_name)}
                      </Link>
                    )}
  ...
  if (mypart !== undefined) {
    return (
      <>
        <div style={{ height: CARDHEIGHT }} className={`card-panel match-card`}>
          <div
          ...
          >
            <ParticipantItems part={mypart} match={match} />
          </span>
          ...
          >
            ...
            <div style={{ fontSize: "small" }}>
              {mypart.stats.kills} / {mypart.stats.deaths} /{" "}
              {mypart.stats.assists}
            </div>
          ...
          >
            <small className={`${matchHighlightColor(match.queue_id)}`}>
              {props.queues[match.queue_id] && (
                <Fragment>
                  {convertQueue(props.queues[match.queue_id].description)}
                </Fragment>
              )}
