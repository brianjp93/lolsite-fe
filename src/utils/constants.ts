import type { ValueOf } from "next/dist/shared/lib/constants";
import api from "@/external/api/api";
import { profileRoute } from "@/routes";

export const ARENA_QUEUE = 1700;

export const REGIONS = [
  "na",
  "euw",
  "eune",
  "kr",
  "jp",
  "lan",
  "las",
  "br",
  "oce",
  "tr",
  "ru",
] as const;
export type Region = ValueOf<typeof REGIONS>;

export const QUEUEFILTER = [
  {
    name: "5v5 Ranked Solo/Duo",
    id: 420,
  },
  {
    name: "5v5 Ranked Flex",
    id: 440,
  },
  {
    name: "5v5 Norms Draft",
    id: 400,
  },
  {
    name: "5v5 Norms Blind",
    id: 430,
  },
  {
    name: "Custom Games",
    id: 0,
  },
  {
    name: "Clash",
    id: 700,
  },
  {
    name: "ARAM Clash",
    id: 720,
  },
  {
    name: "ARAM",
    id: 450,
  },
];

export const QUEUE_CONVERT: Record<string, string> = {
  RANKED_SOLO_5x5: "Solo/Duo",
  RANKED_FLEX_SR: "5v5 Flex",
  RANKED_FLEX_TT: "3v3 Flex",
  RANKED_TFT: "TFT",
  RANKED_TFT_DOUBLE_UP: "TFT Double Up",
} as const;

function getStatModImageUrl({ patch = "latest", key = "" }) {
  return `https://raw.communitydragon.org/${patch}/game/assets/perks/statmods/${key}.png`;
}

// https://raw.communitydragon.org/14.6/plugins/rcp-be-lol-game-data/global/en_gb/v1/perks.json
export function getStatMod(patch = "latest") {
  const data = {
    statmodshealthscalingicon: {
      name: "65 Bonus Health",
      key: "statmodshealthscalingicon",
      id: 5011,
      image_url: getStatModImageUrl({
        patch,
        key: "statmodshealthscalingicon",
      }),
    },
    statmodshealthplusicon: {
      name: "+10-180 Health (based on level)",
      key: "statmodshealthplusicon",
      id: 5001,
      image_url: getStatModImageUrl({
        patch,
        key: "statmodshealthplusicon",
      }),
    },
    statmodsarmoricon: {
      name: "+6 Armor",
      key: "statmodsarmoricon",
      id: 5002,
      image_url: getStatModImageUrl({ patch, key: "statmodsarmoricon" }),
    },
    statmodsmagicresicon: {
      name: "+8 Magic Resist",
      key: "statmodsmagicresicon",
      id: 5003,
      image_url: getStatModImageUrl({
        patch,
        key: "statmodsmagicresicon",
      }),
    },
    statmodsattackspeedicon: {
      name: "+10% Attack Speed",
      key: "statmodsattackspeedicon",
      id: 5005,
      image_url: getStatModImageUrl({ patch, key: "statmodsattackspeedicon" }),
    },
    statmodsadaptiveforceicon: {
      name: "Adaptive Force +9",
      key: "statmodsadaptiveforceicon",
      id: 5008,
      image_url: getStatModImageUrl({
        patch,
        key: "statmodsadaptiveforceicon",
      }),
    },
    statmodscdrscalingicon: {
      name: "+1-10% CDR (based on level)",
      key: "statmodscdrscalingicon",
      id: 5007,
      image_url: getStatModImageUrl({ patch, key: "statmodscdrscalingicon" }),
    },
    statmodstenacityicon: {
      name: "10% tenacity and slow resist",
      key: "statmodstenacityicon",
      id: 5013,
      image_url: getStatModImageUrl({ patch, key: "statmodstenacityicon" }),
    },
    statmodsmovementspeedicon: {
      name: "2% bonus movement speed",
      key: "statmodsmovementspeedicon",
      id: 5010,
      image_url: getStatModImageUrl({ patch, key: "statmodsmovementspeedicon" }),
    },
  };
  return data;
}

export const RUNEDATA = {
  data: {
    "8005": {
      id: 8005,
      name: "Press the Attack",
      perkVars: 3,
      perkDesc: ["Total Damage", "Bonus Damage", "Expose Damage"],
      perkFormat: ["{0}", "{0}", "{0}"],
    },
    "8008": {
      id: 8008,
      name: "Lethal Tempo",
      perkVars: 2,
      perkDesc: ["Total Active Time"],
      perkFormat: ["{0}:{1}"],
    },
    "8009": {
      id: 8009,
      name: "Presence of Mind",
      perkVars: 1,
      perkDesc: ["Total Mana Restored"],
      perkFormat: ["{0}"],
    },
    "8010": {
      id: 8010,
      name: "Conqueror",
      perkVars: 1,
      perkDesc: ["Healed"],
      perkFormat: ["{0}"],
    },
    "8014": {
      id: 8014,
      name: "Coup de Grace",
      perkVars: 1,
      perkDesc: ["Total Bonus Damage"],
      perkFormat: ["{0}"],
    },
    "8017": {
      id: 8017,
      name: "Cut Down",
      perkVars: 1,
      perkDesc: ["Total Bonus Damage"],
      perkFormat: ["{0}"],
    },
    "8021": {
      id: 8021,
      name: "Fleet Footwork",
      perkVars: 1,
      perkDesc: ["Total Healing"],
      perkFormat: ["{0}"],
    },
    "8105": {
      id: 8105,
      name: "Relentless Hunter",
      perkVars: 2,
      perkDesc: ["Movement Speed Increase", "Total Stacks"],
      perkFormat: ["{0}%", "{0}"],
    },
    "8106": {
      id: 8106,
      name: "Ultimate Hunter",
      perkVars: 1,
      perkDesc: ["Total Stacks"],
      perkFormat: ["{0}"],
    },
    "8112": {
      id: 8112,
      name: "Electrocute",
      perkVars: 1,
      perkDesc: ["Total Damage Dealt"],
      perkFormat: ["{0}"],
    },
    "8120": {
      id: 8120,
      name: "Ghost Poro",
      perkVars: 1,
      perkDesc: ["Poros Spawned"],
      perkFormat: ["{0}"],
    },
    "8124": {
      id: 8124,
      name: "Predator",
      perkVars: 1,
      perkDesc: ["Total Damage to Champions"],
      perkFormat: ["{0}"],
    },
    "8126": {
      id: 8126,
      name: "Cheap Shot",
      perkVars: 1,
      perkDesc: ["Total Damage"],
      perkFormat: ["{0}"],
    },
    "8128": {
      id: 8128,
      name: "Dark Harvest",
      perkVars: 2,
      perkDesc: ["Total Damage Dealt", "Total Souls Harvested"],
      perkFormat: ["{0}", "{0}"],
    },
    "8134": {
      id: 8134,
      name: "Ingenious Hunter",
      perkVars: 1,
      perkDesc: ["Total Stacks"],
      perkFormat: ["{0}"],
    },
    "8135": {
      id: 8135,
      name: "Ravenous Hunter",
      perkVars: 2,
      perkDesc: ["Total Bonus Healing", "Total Stacks"],
      perkFormat: ["{0}", "{0}"],
    },
    "8136": {
      id: 8136,
      name: "Zombie Ward",
      perkVars: 1,
      perkDesc: ["Wards Corrupted"],
      perkFormat: ["{0}"],
    },
    "8138": {
      id: 8138,
      name: "Eyeball Collection",
      perkVars: 1,
      perkDesc: ["Total Bonus AD/AP"],
      perkFormat: ["{0}"],
    },
    "8139": {
      id: 8139,
      name: "Taste of Blood",
      perkVars: 1,
      perkDesc: ["Total Healing"],
      perkFormat: ["{0}"],
    },
    "8143": {
      id: 8143,
      name: "Sudden Impact",
      perkVars: 1,
      perkDesc: ["Total Damage"],
      perkFormat: ["{0}"],
    },
    "8210": {
      id: 8210,
      name: "Transcendence",
      perkVars: 1,
      perkDesc: ["Total Adaptive Stat Gained"],
      perkFormat: ["{0}"],
    },
    "8214": {
      id: 8214,
      name: "Summon Aery",
      perkVars: 2,
      perkDesc: ["Damage Dealt", "Damage Shielded"],
      perkFormat: ["{0}", "{0}"],
    },
    "8224": {
      id: 8224,
      name: "Nullifying Orb",
      perkVars: 1,
      perkDesc: ["Total Shield Granted"],
      perkFormat: ["{0}"],
    },
    "8226": {
      id: 8226,
      name: "Manaflow Band",
      perkVars: 2,
      perkDesc: ["Total Resource Refunded", "Total Mana Restored"],
      perkFormat: ["{0}", "{0}"],
    },
    "8229": {
      id: 8229,
      name: "Arcane Comet",
      perkVars: 1,
      perkDesc: ["Total Damage Dealt"],
      perkFormat: ["{0}"],
    },
    "8230": {
      id: 8230,
      name: "Phase Rush",
      perkVars: 1,
      perkDesc: ["Total Activations"],
      perkFormat: ["{0}"],
    },
    "8232": {
      id: 8232,
      name: "Waterwalking",
      perkVars: 1,
      perkDesc: ["Total Bonus AD/AP"],
      perkFormat: ["{0}"],
    },
    "8233": {
      id: 8233,
      name: "Absolute Focus",
      perkVars: 2,
      perkDesc: ["Total Time Active"],
      perkFormat: ["{0}:{1}"],
    },
    "8234": {
      id: 8234,
      name: "Celerity",
      perkVars: 1,
      perkDesc: ["Extra Distance Traveled"],
      perkFormat: ["{0}"],
    },
    "8236": {
      id: 8236,
      name: "Gathering Storm",
      perkVars: 1,
      perkDesc: ["Total Bonus AD/AP"],
      perkFormat: ["{0}"],
    },
    "8237": {
      id: 8237,
      name: "Scorch",
      perkVars: 1,
      perkDesc: ["Total Bonus Damage"],
      perkFormat: ["{0}"],
    },
    "8242": {
      id: 8242,
      name: "Unflinching",
      perkVars: 1,
      perkDesc: ["Bonus Tenacity per Activation"],
      perkFormat: ["{0}"],
    },
    "8243": {
      id: 8243,
      name: "The Ultimate Hat",
      perkVars: 1,
      perkDesc: ["Total Bonus CDR"],
      perkFormat: ["{0}"],
    },
    "8275": {
      id: 8275,
      name: "Nimbus Cloak",
      perkVars: 1,
      perkDesc: ["Times Activated"],
      perkFormat: ["{0}"],
    },
    "8299": {
      id: 8299,
      name: "Last Stand",
      perkVars: 1,
      perkDesc: ["Total Bonus Damage"],
      perkFormat: ["{0}"],
    },
    "8304": {
      id: 8304,
      name: "Magical Footwear",
      perkVars: 2,
      perkDesc: ["Boots Arrival Time"],
      perkFormat: ["{0}:{1}"],
    },
    "8306": {
      id: 8306,
      name: "Hextech Flashtraption",
      perkVars: 1,
      perkDesc: ["Times Hexflashed"],
      perkFormat: ["{0}"],
    },
    "8313": {
      id: 8313,
      name: "Perfect Timing",
      perkVars: 0,
      perkDesc: ["-"],
      perkFormat: [""],
    },
    "8316": {
      id: 8316,
      name: "Minion Dematerializer",
      perkVars: 1,
      perkDesc: ["Bonus Minion Damage Dealt"],
      perkFormat: ["{0}"],
    },
    "8321": {
      id: 8321,
      name: "Future's Market",
      perkVars: 1,
      perkDesc: ["Future Purchases"],
      perkFormat: ["{0}"],
    },
    "8326": {
      id: 8326,
      name: "Unsealed Spellbook",
      perkVars: 1,
      perkDesc: ["Summoners Swapped"],
      perkFormat: ["{0}"],
    },
    "8339": {
      id: 8339,
      name: "Celestial Body",
      perkVars: 0,
      perkDesc: ["-"],
      perkFormat: [""],
    },
    "8345": {
      id: 8345,
      name: "Biscuit Delivery",
      perkVars: 1,
      perkDesc: ["Biscuits Received"],
      perkFormat: ["{0}"],
    },
    "8347": {
      id: 8347,
      name: "Cosmic Insight",
      perkVars: 0,
      perkDesc: ["-"],
      perkFormat: ["{0}"],
    },
    "8351": {
      id: 8351,
      name: "Glacial Augment",
      perkVars: 1,
      perkDesc: ["Bonus Time Enemies Slowed"],
      perkFormat: ["{0}"],
    },
    "8352": {
      id: 8352,
      name: "Timewarp Tonic",
      perkVars: 3,
      perkDesc: [
        "Time Spent Hasted",
        "Total Immediate Health Restored",
        "Total Immediate Mana",
      ],
      perkFormat: ["{0}", "{0}", "{0}"],
    },
    "8358": {
      id: 8358,
      name: "Omnistone",
      perkVars: 1,
      perkDesc: ["Keystones Used"],
      perkFormat: ["{0}"],
    },
    "8359": {
      id: 8359,
      name: "Kleptomancy",
      perkVars: 2,
      perkDesc: ["Gold Granted", "Items Looted"],
      perkFormat: ["{0}", "{0}"],
    },
    "8360": {
      id: 8360,
      name: "Unsealed Spellbook",
      perkVars: 1,
      perkDesc: ["Summoners Swapped"],
      perkFormat: ["{0}"],
    },
    "8369": {
      id: 8369,
      name: "First Strike",
      perkVars: 2,
      perkDesc: ["Bonus Damage", "Gold Gained"],
      perkFormat: ["{0}", "{0}"],
    },
    "8401": {
      id: 8410,
      name: "Shield Bash",
      perkVars: 1,
      perkDesc: ["Damage Dealt"],
      perkFormat: ["{0}"],
    },
    "8410": {
      id: 8410,
      name: "Approach Velocity",
      perkVars: 1,
      perkDesc: ["Time Spent Hasted"],
      perkFormat: ["{0}"],
    },
    "8429": {
      id: 8429,
      name: "Conditioning",
      perkVars: 3,
      perkDesc: [
        "Percent of game active",
        "Total Bonus Armor",
        "Total Bonus Magic Resist",
      ],
      perkFormat: ["{0}", "{0}", "{0}"],
    },
    "8430": {
      id: 8430,
      name: "Iron Skin",
      perkVars: 1,
      perkDesc: ["Percent of Game Active"],
      perkFormat: ["{0}"],
    },
    "8435": {
      id: 8435,
      name: "Mirror Shell",
      perkVars: 1,
      perkDesc: ["Bonus Magic Resist"],
      perkFormat: ["{0}"],
    },
    "8437": {
      id: 8437,
      name: "Grasp of the Undying",
      perkVars: 2,
      perkDesc: ["Total Damage", "Total Healing"],
      perkFormat: ["{0}", "{0}"],
    },
    "8439": {
      id: 8439,
      name: "Aftershock",
      perkVars: 1,
      perkDesc: ["Total Damage Dealt"],
      perkFormat: ["{0}"],
    },
    "8444": {
      id: 8444,
      name: "Second Wind",
      perkVars: 1,
      perkDesc: ["Total Healing"],
      perkFormat: ["{0}"],
    },
    "8446": {
      id: 8446,
      name: "Demolish",
      perkVars: 1,
      perkDesc: ["Total Bonus Damage"],
      perkFormat: ["{0}"],
    },
    "8451": {
      id: 8451,
      name: "Overgrowth",
      perkVars: 1,
      perkDesc: ["Total Bonus Max Health"],
      perkFormat: ["{0}"],
    },
    "8453": {
      id: 8453,
      name: "Revitalize",
      perkVars: 2,
      perkDesc: ["Bonus Healing", "Bonus Shielding"],
      perkFormat: ["{0}", "{0}"],
    },
    "8463": {
      id: 8463,
      name: "Font of Life",
      perkVars: 1,
      perkDesc: ["Total Ally Healing"],
      perkFormat: ["{0}"],
    },
    "8465": {
      id: 8465,
      name: "Guardian",
      perkVars: 1,
      perkDesc: ["Total Shield Strength"],
      perkFormat: ["{0}"],
    },
    "8473": {
      id: 8473,
      name: "Bone Plating",
      perkVars: 1,
      perkDesc: ["Damage Mitigated"],
      perkFormat: ["{0}"],
    },
    "9101": {
      id: 9101,
      name: "Overheal",
      perkVars: 1,
      perkDesc: ["Total Shielding"],
      perkFormat: ["{0}"],
    },
    "9103": {
      id: 9103,
      name: "Legend: Bloodline",
      perkVars: 2,
      perkDesc: ["Time Completed"],
      perkFormat: ["{0}:{1}"],
    },
    "9104": {
      id: 9104,
      name: "Legend: Alacrity",
      perkVars: 2,
      perkDesc: ["Time Completed"],
      perkFormat: ["{0}:{1}"],
    },
    "9105": {
      id: 9105,
      name: "Legend: Tenacity",
      perkVars: 2,
      perkDesc: ["Time Completed"],
      perkFormat: ["{0}:{1}"],
    },
    "9111": {
      id: 9111,
      name: "Triumph",
      perkVars: 2,
      perkDesc: ["Total Health Restored", "Total Bonus Gold"],
      perkFormat: ["{0}", "{0}"],
    },
    "9923": {
      id: 9923,
      name: "Hail of Blades",
      perkVars: 1,
      perkDesc: ["Times Activated"],
      perkFormat: ["{0}"],
    },
  },
};

export function getRiotIdAndTaglineFromSearchName(searchName: string) {
  let [name, tagline] = searchName.split("-");
  name = name || "";
  tagline = tagline || "";
  return [name, tagline] as const;
}

export async function getProfileRouteFromPuuid(puuid: string, region: string) {
  const response = await api.player.getSummoner({ puuid, region });
  return profileRoute({
    region: region,
    riotIdName: response.riot_id_name,
    riotIdTagline: response.riot_id_tagline,
  });
}
